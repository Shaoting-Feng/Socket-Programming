...
int main(int argc, char *argv[])
{
    ...
    int total = 0, ttotal, usefultotal = 0;
    clock_t tstart, tend;
    char s_buf;
    char *send_buf = &s_buf;
    int lastsign = -1, thissign;
    ...
    if ((ttotal = recvfrom(sockfd, buf+total, maxsize, 0,
        ...
        s_buf = *(buf+total);
        thissign = *(buf+total) - '0';
        if ((numbytes = sendto(sockfd, send_buf, sizeof(char), 0,
                p->ai_addr, p->ai_addrlen)) == -1) {
            perror("talker: sendto");
            exit(1);
        }
    ...
    printf("listener: got packet from %s\n",
                ...
                s, sizeof s));
    printf("listener: packet is %d bytes long(including duplicate packets and serial numbers)\n", total);
    printf("listener: packet is %d bytes long(duplicate packets or serial numbers not included)\n", usefultotal);
    // actually, it can be modified to record only useful packets
    buf[total] = '\0';
